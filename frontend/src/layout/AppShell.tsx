...
export function AppShell({ admin }: { admin: Admin }) {
  ...
  const content = page === 1 ? <BacktestBuilder /> : page === 2 ? <BacktestResults /> : page === 0 ? <OverviewPage onCreate={() => setPage(1)} /> : <section className="panel quiet-empty">此功能將在本次正式版後續階段啟用。</section>;
